...
```typescript
// ✅ Pass Server Component to Client Component
// ClientWrapper.tsx
'use client'
...
```typescript
// ✅ Custom Hook for Logic (Container)
...
```typescript
// ✅ Flexible Parent/Child Components
...
```typescript
// ✅ Prop Collections for Complex Components
function useToggle() {
  const [on, setOn] = useState(false)
  const toggle = () => setOn(!on)
...
```typescript
// ✅ Debounce rapid events (like search)
