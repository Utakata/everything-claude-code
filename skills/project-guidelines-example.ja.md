...
これはプロジェクト固有のスキルの例である。これをテンプレートとして自身のプロジェクトに使用すること。

実際の本番アプリケーションに基づく: [Zenith](https://zenith.chat) - AI搭載の顧客発見プラットフォーム。

---

## 使用するタイミング

このスキルが設計された特定のプロジェクトで作業する際に参照する。プロジェクトスキルには以下が含まれる：
- アーキテクチャ概要
- ファイル構造
- コードパターン
- テスト要件
...
2. **不変性** - オブジェクトや配列を決して変更（ミューテーション）しない
3. **TDD** - 実装の前にテストを書く
4. **80% カバレッジ** 最低限
5. **多数の小ファイル** - 通常200-400行、最大800行
6. 本番コードに **console.log なし**
7. try/catch による **適切なエラーハンドリング**
