...
async function main() {
  console.log('[SCAN] Scanning for missing ja-JP translations...');
  const missing = collectMissingFiles();

  if (missing.length === 0) {
    console.log('[OK] All files are translated. Nothing to do.');
    if (OUT_LIST) fs.writeFileSync(OUT_LIST, '');
    return;
  }

  console.log(`[INFO] Found ${missing.length} untranslated file(s).`);
  const toProcess = missing.slice(0, MAX_FILES);
  console.log(`[PROC] Processing ${toProcess.length} file(s) (max: ${MAX_FILES}).\n`);
  ...
  console.log(`\n[STAT] Done: ${ok} ok, ${ng} failed, ${missing.length - toProcess.length} deferred.`);
