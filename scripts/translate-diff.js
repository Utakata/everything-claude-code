...
async function main() {
  ...
  if (missing.length === 0) {
    console.log('[v] All files are translated. Nothing to do.');
  ...
  console.log(`[gear]  Processing ${toProcess.length} file(s) (max: ${MAX_FILES}).\n`);
  ...
  console.log(`\n[chart] Done: ${ok} ok, ${ng} failed, ${missing.length - toProcess.length} deferred.`);
  if (missing.length > MAX_FILES) {
    console.log(`[i]  ${missing.length - MAX_FILES} file(s) remain — next run will pick them up.`);
  }
